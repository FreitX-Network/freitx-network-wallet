...
func (s *WalletServer) NewWallet(ctx context.Context, in *pb.NewWalletRequest) (*pb.NewWalletResponse, error) {
	...
	addr, err := OnexAddress.NewAddress(OnexAddress.IsTestnet, chainIDBytes[:])
	if err != nil {
		logger.Error().Err(err).Msg("error when creating a new address")
		return nil, errors.Wrap(err, "failed to generate a new OnexAddress")
	}
	...
	creatorAddr, err := OnexAddress.GetAddressByPubkey(OnexAddress.IsTestnet, chainIDBytes[:], publicKey)
	if err != nil {
...
func (s *WalletServer) Unlock(ctx context.Context, in *pb.UnlockRequest) (*pb.UnlockResponse, error) {
	...
	var chainIDBytes [4]byte
	enc.MachineEndian.PutUint32(chainIDBytes[:], uint32(in.ChainID))
	address, err := OnexAddress.GetAddressByPubkey(OnexAddress.IsTestnet, chainIDBytes[:], publicKey)
	if err != nil {
...
func (s *WalletServer) DecodeAddress(ctx context.Context, in *pb.DecodeAddressRequest) (*pb.DecodeAddressResponse, error) {
	...
	hash, err := OnexAddress.GetPubkeyHash(in.Address)
	if err != nil {
	...
	return &pb.DecodeAddressResponse{
		...
		IsTestnet: OnexAddress.IsTestnet,
...
func pbToAddress(addressPb *pb.Address) (*OnexAddress.Address, error) {
